...
init(Parent, Free) ->
    ...
    case proc_lib:start_link(rh, init_dh, [self(), {Free, []}]) of
	{ok, Dh_pid} ->
...
active(Parent, Deb) -> 
	...
	{system, From, Request} ->
	    ...
	    receive
		#rh_reply_data{data=#data_structure{free=Free, allocated=Allocated}} ->
