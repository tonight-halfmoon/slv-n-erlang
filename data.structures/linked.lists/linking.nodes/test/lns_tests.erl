...
api_from_list_test_() ->
    {
      ...
      ?_assertEqual({lns,{node,v1,{node,v2,{node,v3,undefined,{time_visited,0}},{time_visited,0}},{time_visited,0}}}, lns:from_list(['v1', 'v2', 'v3']))
    }.
...
api_to_list_test_() ->
    {
      ...
      ?_assertMatch([{v1, T1},
                      {v2, T2},
                      {v3, T3}] when T1 > 0; T2 > 0; T3 > 0, lns:to_list({lns,{node,v1,{node,v2,{node,v3,undefined,{time_visited,0}},{time_visited,0}},{time_visited,0}}}))
    }.
...
api_tail_test_() ->
	...
	fun(Lns) ->
		[?_assertEqual({node, v1, undefined, {time_visited, 0}}, lns:tail(Lns))]
	end
      }
    }.
...
api_visit_all_test_() ->
	...
	fun() -> setup(3) end,
	fun(Lns) ->
		[?_assertMatch({lns,{node,v1, {node,v2, {node,v3,undefined,{time_visited,T1}},{time_visited,T2}}, {time_visited,T3}}} when T1 > 0; T2 > 0; T3 > 0, lns:visit_all(Lns))]
	end
      }
    }.
...
api_pop_test_() ->
	...
	fun() -> setup(3) end,
	fun(Lns) ->
		[?_assertEqual({v1,{lns,{node,v2,{node,v3,undefined,{time_visited,0}},{time_visited,0}}}}, lns:pop(Lns))]
	end
      }
    }.
